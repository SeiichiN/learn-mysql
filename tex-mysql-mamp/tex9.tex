...
また、その中に、teizanテーブルと stateテーブルがあります。

まず、mountain\_user ユーザーを作成し、
mountain\_user に mountain データベースへの権限を与えてください。

\begin{lstlisting}[caption=作業例, numbers=none, language=sql]
 > mysql -u root -p
 Password: ********
 mysql> GRANT ALL ON mountain.* TO 'mountain_user'@'localhost'
     -> IDENTIFIED BY '1234';
 mysql> exit
\end{lstlisting}

次に、mountain\_userユーザーでログインし、mountain.sql を読み込んでください。

\begin{lstlisting}[caption=作業例, numbers=none, language=sql]
 > mysql -u mountain_user -p
 Password: *******
 mysql> source mountain.sql     (あるいは)    \. mountain.sql
\end{lstlisting}
...
\fbox{gender表} \fbox{person表} \fbox{person\_teizan表} \\
...
\begin{enumerate}
 ...
 \item teizan表の所在地の項目は、state表の主キーを参照している。
...
\begin{tabular}{|c|l|c|l|} \hline
 id & name           & g\_id & birthday   \\ \hline\hline
...
\begin{lstlisting}[caption=person\_teizan, numbers=none]
+-------+---------+------+-----+---------+-------+
