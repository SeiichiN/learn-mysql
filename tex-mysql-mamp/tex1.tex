...
\begin{enumerate}
 \item MAMP を起動する。
 \item Start Servers ボタンをクリックして MySQLを起動する。
\end{enumerate}

\vspace{3mm}
\includegraphics[width=10cm]{../07-mamp/03-start-mamp.png}
\vspace{3mm}
...
これで、8行ほどのメッセージと、次のプロンプトが表示される。

\begin{tcolorbox}
 mysql$>$
\footnote{今までは windows のコマンドプロンプト環境で、プロンプトは $>$
であった。mysql$>$ というプロンプトは MySQLデータベースにログインしている
、つまりMySQLの環境にいるということである。だから、コマンドプロンプトでの
コマンドは使えないことに注意する。}
\end{tcolorbox}
...
以下のコマンドで \textsf{sampleuser} を作成し、\textsf{1234} というパスワードを設定し、
\textsf{sample} データベースへの権限を与えることができる。
\footnote{ユーザーの作成と権限付与を別々にすることもできる。\\
sampleuser というユーザーの作成と 1234 というパスワードの設定\\
\hspace{3mm}
 mysql$>$ CREATE USER 'sampleuser'@'localhost' IDENTIFIED BY '1234'; \\
sampleuser に sample データベースへの権限を付与する。\\
\hspace{3mm}
 mysql$>$ GRANT ALL [PRIVILEGES] ON sample.* TO 'sampleuser'@'localhost';
}
ここでは、ユーザー名を sampleuser、パスワードを 1234 としている。

\begin{tcolorbox}
mysql$>$ GRANT \ ALL \ [PRIVILEGES]
 \footnote{[PRIVILEGES]は省略できる。}
 \ ON \ sample.* \ TO \ 'sampleuser'@'localhost' \\
\hspace{20mm} -$>$ IDENTIFIED \ BY \ '1234';
\end{tcolorbox}
...
あるいは、このように -p の後に続けてパスワードを書くこともできる。
(空白は{\textgt なし}で)\\
\fbox{mysql \ -u \ ユーザー名 \ -pパスワード}
\end{multicols}
\vspace{3mm}
...
\begin{tcolorbox}
 mysql$>$ \textsf{create database sample;}
\end{tcolorbox}
