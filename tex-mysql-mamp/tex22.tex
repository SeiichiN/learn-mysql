...
\begin{tcolorbox}
 \verb!mysql> SHOW VARIABLES LIKE '%char%';!
\end{tcolorbox}
...
\verb!| character_sets_dir       | C:\MAMP\bin\mysql\share\charsets\ |! \\ 
\verb!+--------------------------+-----------------------------------+! \\ \hline
\end{tabular}
\vspace{3mm}
...
\verb!| Variable_name            | Value                             |! \\
\verb!+--------------------------+-----------------------------------+! \\
\verb!| character_set_client     | utf8                              |! \\
\verb!| character_set_connection | utf8                              |! \\
\verb!| character_set_database   | utf8mb4                           |! \\
\verb!| character_set_filesystem | binary                            |! \\
\verb!| character_set_results    | utf8                              |! \\
\verb!| character_set_server     | utf8mb4                           |! \\
\verb!| character_set_system     | utf8                              |! \\
\verb!| character_sets_dir       | C:\MAMP\bin\mysql\share\charsets\ |! \\ 
\verb!+--------------------------+-----------------------------------+! \\ \hline
...
\begin{tcolorbox}
 mysql$>$ SHOW \ VARIABLES \ LIKE \  \verb!'%char%'!;
\end{tcolorbox}
...
MAMPコントロールパネルで ``Start Severs'' ボタンをクリックしがのは、
サーバープログラム(mysqld.exe)を起動しているのである。

コマンドプロンプトで ``mysql -u sampleuser -p'' としているのは、
クライアントプログラム(mysql.exe)を使って、サーバープログラムに接続し、
ログイン処理をおこなっているのである。

普通はサーバーはネットワーク上のどこか離れた場所にあるのだけれど、
MAMPでは、各自のパソコン内でサーバープログラムと
...
character\_sets\_dir       & : &
     C:\yen MAMP\yen bin\yen mysql\yen share\yen charsets\yen \\ 
\end{tabular}
